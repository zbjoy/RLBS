...
StoreReport::StoreReport()
{
	...
	string db_user = config_file::instance()->GetString("mysql", "db_user", "123");
...
void StoreReport::store(rlbs::ReportStatusRequest& req)
{
	for (int i = 0; i < req.results_size(); i++)	
	{
		const rlbs::HostCallResult& result = req.results(i);		
		...
		snprintf(sql, 1024, "INSERT INTO ServerCallStatus"
							...
							"UPDATE succ_cnt = %d, err_cnt = %u, ts = %u, overload = %d",
							req.modid(), req.cmdid(), result.ip(), result.port(), req.caller(), result.succ(), result.err(), req.ts(), overload, result.succ(), result.err(), req.ts(), overload);

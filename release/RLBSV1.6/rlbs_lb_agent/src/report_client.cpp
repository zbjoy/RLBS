...
void new_report_request(event_loop *loop, int fd, void *args)
{
    ...
    std::queue<rlbs::ReportStatusRequest>  msgs;
    ...
    while ( !msgs.empty() ) {
        rlbs::ReportStatusRequest req = msgs.front();
        msgs.pop();
        ...
        //将这个消息透传给dns service
        client->send_message(requestString.c_str(), requestString.size(), rlbs::ID_ReportStatusRequest);
    }
}
